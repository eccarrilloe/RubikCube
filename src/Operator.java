    ...
    switch (operation) {
      case OP_ROTATE_FRONT:
        if (direction == DIR_RIGHT) {
          cube.frontSide.rotate(DIR_RIGHT);//ok
          tmp = cube.topSide.getRow(ROW_BOTTOM);
          cube.topSide.setRow(ROW_BOTTOM, cube.leftSide.getColumn(COL_RIGHT), true);//ok
          cube.leftSide.setColumn(COL_RIGHT, cube.bottomSide.getRow(ROW_TOP), false);//ok
          cube.bottomSide.setRow(ROW_TOP, cube.rightSide.getColumn(COL_LEFT), true);//ok
          cube.rightSide.setColumn(COL_LEFT, tmp, false);//ok
        } else {
          cube.frontSide.rotate(DIR_LEFT);//ok
          tmp = cube.topSide.getRow(ROW_BOTTOM);//ok tmp
          cube.topSide.setRow(ROW_BOTTOM, cube.rightSide.getColumn(COL_LEFT), false);//ok
          cube.rightSide.setColumn(COL_LEFT, cube.bottomSide.getRow(ROW_TOP), true);//ok now.
          cube.bottomSide.setRow(ROW_TOP, cube.leftSide.getColumn(COL_RIGHT), false);//ok
          cube.leftSide.setColumn(COL_RIGHT, tmp, true);//ok
        }
        break;
      case OP_ROTATE_BACK:
        if (direction == DIR_RIGHT) {
          ...
          tmp = cube.topSide.getRow(ROW_TOP);
          cube.topSide.setRow(ROW_TOP, cube.rightSide.getColumn(COL_LEFT), true);
          cube.rightSide.setColumn(COL_LEFT, cube.bottomSide.getRow(ROW_BOTTOM), false);
          cube.bottomSide.setRow(ROW_BOTTOM, cube.leftSide.getColumn(COL_RIGHT), true);
          cube.leftSide.setColumn(COL_RIGHT, tmp, false);
        }
        break;
      case OP_ROW_TOP:
        if (direction == DIR_RIGHT) {
